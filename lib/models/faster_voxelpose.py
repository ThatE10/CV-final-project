...
from lib.models.human_detection_net import HumanDetectionNet
from lib.models.joint_localization_net import JointLocalizationNet
...
class FasterVoxelPoseNet(nn.Module):
    ...
    def forward(self, backbone=None, views=None, meta=None, targets=None, input_heatmaps=None, cameras=None, resize_transform=None):
        ...
        # pad missing channels if needed
        num_expected_channels = 15
        num_current_channels = input_heatmaps.shape[2]
        num_pad = num_expected_channels - num_current_channels
        if num_pad > 0:
            pad_tensor = torch.zeros(
                (input_heatmaps.shape[0], input_heatmaps.shape[1], num_pad, input_heatmaps.shape[3],
                 input_heatmaps.shape[4]),
                device=input_heatmaps.device,
                dtype=input_heatmaps.dtype
            )
            input_heatmaps = torch.cat([input_heatmaps, pad_tensor], dim=2)
